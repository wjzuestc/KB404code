...
figure;
imagesc(real(RawData));colormap(gray);title('原始回波');xlabel('距离向');ylabel('方位向');
figure('Name','回波');
imagesc(abs(fty(ftx(RawData)))); title('原始回波频谱');xlabel('距离向');ylabel('方位向');%频谱图
%% parameter analysis
...
fdr=-Vr*Vr.'*cos(theta_Rsq)^2/lambda/...
    ...
    sqrt((TargetCenter-Location_T)*(TargetCenter-Location_T).');
%% 去模糊
dataRR=(RawData.'.*exp(-1i*2*pi*fdc*ones(RanNum,1)*Ta)).';
figure;imagesc(real(dataRR));colormap(gray);
figure('Name','去模糊的回波');
imagesc(abs(fty(ftx(dataRR))));title('方位去模糊后的频谱');xlabel('距离向');ylabel('方位向')
%%
%距离向脉压
hr=fftshift((abs(Tr)<Tp/2).*(0.54-0.46*cos(2*pi*(1:RanNum)/RanNum)).*...
        exp(cj*pi*Kr*(Tr).^2));
Srancom=fft(dataRR.').*conj(fft(hr.')*ones(1,AziNum));
...
figure;imagesc(abs(ss));colormap(gray);title('RCMC');xlabel('距离向');ylabel('方位向')
%% 取一个距离门
fdc_process_data=ss(1699:2399,2049);
figure;plot(imag(fdc_process_data));xlabel('方位向');ylabel('幅度')
figure;plot(real(fdc_process_data));xlabel('方位向');ylabel('幅度')
%%
% step = 0.001;
% stepNum = size(real_fdc_data,1)/step;
...
%% 找点
count = 0;
for k=1:size(chafen,2)-3
    if abs(chafen(k))<20 && abs(chafen(k+1))<20 && abs(chafen(k+2))<20
        count = count+1;
    else
        count = 0;
    end
