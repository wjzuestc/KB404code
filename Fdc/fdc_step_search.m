...
% 算法运行时间优化
% [val,RanSelect] = max(max(real(sss)));
% fdc_process_data_have_fdc=sss(:,RanSelect);
% figure;plot(imag(fdc_process_data_have_fdc));
% %对负数来说 .'才是单纯转置，’是共轭转置
% fdc_process_data = fdc_process_data_have_fdc.'.*(exp(-1i*2*pi*720*Ta));
...
% figure;plot(imag(fdc_process_data))
% figure;plot(real(fdc_process_data))
real_fdc_data_la = 1./abs(fdc_process_data).*fdc_process_data;
real_fdc_data = imag(real_fdc_data_la)+2000;
% real_fdc_data = imag(fdc_process_data)+2000;
% figure;plot(real_fdc_data);
% gauss = awgn(real_fdc_data-2000,20,'measured');
%figure;plot(gauss)
...
% 遍历曲线
figure;plot(fdc,NumSum(2:end));xlabel('粗估fdc值');ylabel('Z(fdc)')
[x,index] = min(NumSum(2:end));
fdc_estimate1 = fdc(index)
%% 中估 参数
step2 = 1;
fdc = [fdc_estimate1-step1:step2:fdc_estimate1+step1];
NumSum = 0;  %fdc遍历值
% 去模糊
for k=1:size(fdc,2)
ss=(sss.'.*exp(-1i*2*pi*fdc(k)*ones(RanNum,1)*Ta)).';
...
[x,index] = min(NumSum(2:end));
fdc_estimate2 = fdc(index)
%% 精估 参数
step3 = 0.1;
fdc = [fdc_estimate2-step2:step3:fdc_estimate2+step2];
NumSum = 0;  %fdc遍历值
% 去模糊
for k=1:size(fdc,2)
ss=(sss.'.*exp(-1i*2*pi*fdc(k)*ones(RanNum,1)*Ta)).';
...
% 画图
figure;plot(fdc,NumSum(2:end));xlabel('精估fdc值');ylabel('Z(fdc)')
[x,index] = min(NumSum(2:end));
fdc_estimate3 = fdc(index)
%% 终极精估 参数
step4 = 0.01;
fdc = [fdc_estimate3-step3:step4:fdc_estimate3+step3];
NumSum = 0;  %fdc遍历值
% 去模糊
for k=1:size(fdc,2)
ss=(sss.'.*exp(-1i*2*pi*fdc(k)*ones(RanNum,1)*Ta)).';
